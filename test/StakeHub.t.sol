...
contract StakeHubTest is Deployer {
    ...
    event MigrateSuccess(address indexed operatorAddress, address indexed delegator, uint256 shares, uint256 amount);
    event MigrateFailed(
    ...
    function testDelegate() public {
        ...
        stakeHub.delegate{ value: amount }(validator, false);
        uint256 shares = IStakeCredit(credit).balanceOf(delegator);
        assertEq(shares, amount);
        uint256 pooledAmount = IStakeCredit(credit).getPooledTokenByShares(shares);
        assertEq(pooledAmount, amount);
    ...
    function testUndelegate() public {
        ...
        vm.expectEmit(true, true, false, true, address(stakeHub));
        emit Claimed(validator, delegator, amount / 2);
        stakeHub.claim(validator, 0);
    ...
    function testUndelegateAll() public {
        ...
        uint256 _totalPooledToken = IStakeCredit(credit).totalPooledToken();
        assertEq(_totalPooledToken, selfDelegation + toLock, "wrong total pooled token");
        ...
        _totalPooledToken = IStakeCredit(credit).totalPooledToken();
        assertEq(_totalPooledToken, toLock, "wrong total pooled token");
    ...
    function testRedelegate() public {
        ...
        uint256 amount = 100 ether;
        stakeHub.delegate{ value: amount }(validator1, false);
        ...
        uint256 redelegateFee = amount * redelegateFeeRate / feeBase;
        uint256 expectedShares = (amount - redelegateFee) * IStakeCredit(credit2).totalSupply()
            / (IStakeCredit(credit2).totalPooledToken() + redelegateFee);
    ...
    function testDistributeReward() public {
        ...
        // 1. delegate 100 token and get 100 * 1e18 shares
        ...
        // preTotalPooledToken: locked amount(1 ether) + selfDelegation(2000 ether) + delegation(100 ether) + (reward - commissionToValidator)(99.9 ether) = 2200.9 ether
        // preTotalShares: locked shares(1 ether) + selfDelegation(2000 ether) + delegation(100 ether)
        // curTotalShares: preTotalShares + commissionToValidator * preTotalShares  / preTotalPooledToken = 2101095460947794084238
        // curTotalPooledToken: preTotalPooledToken + commissionToValidator = 2201 ether
        // expectedTokenAmount: shares(100 ether) * curTotalPooledToken / curTotalShares
        uint256 _totalShares = IStakeCredit(credit).totalSupply();
        assertEq(_totalShares, 2101095460947794084238, "wrong total shares");
        uint256 expectedTokenAmount = shares * 2201 ether / uint256(2101095460947794084238);
        uint256 realTokenAmount = IStakeCredit(credit).getPooledTokenByShares(shares);
        assertEq(realTokenAmount, expectedTokenAmount, "wrong token amount");
        ...
        // totalPooledToken: 2201 ether - (100 ether + 99.9 ether * 100 / 2101 ) = 2096245121370775821038
        // newShares: 100 ether * totalShares / totalPooledToken
        uint256 _totalPooledToken = IStakeCredit(credit).totalPooledToken();
        assertEq(_totalPooledToken, 2096245121370775821038, "wrong total pooled token");
    ...
    function testDowntimeSlash() public {
        ...
        uint256 preDelegatorTokenAmount =
            IStakeCredit(credit).getPooledTokenByShares(IStakeCredit(credit).balanceOf(delegator));
        uint256 preValidatorTokenAmount =
            IStakeCredit(credit).getPooledTokenByShares(IStakeCredit(credit).balanceOf(validator));
        ...
        uint256 curValidatorTokenAmount =
            IStakeCredit(credit).getPooledTokenByShares(IStakeCredit(credit).balanceOf(validator));
        assertApproxEqAbs(preValidatorTokenAmount, curValidatorTokenAmount + slashAmt, 1); // there may be 1 delta due to the precision

        // check delegator's share
        uint256 curDelegatorTokenAmount =
            IStakeCredit(credit).getPooledTokenByShares(IStakeCredit(credit).balanceOf(delegator));
        assertApproxEqAbs(preDelegatorTokenAmount, curDelegatorTokenAmount, 1); // there may be 1 delta due to the precision
    ...
    function testUpdateValidatorSetV2() public {
        ...
        vm.txGasPrice(0);
        validatorSet.updateValidatorSetV2(newConsensusAddrs, newVotingPower, newVoteAddrs);
    }
