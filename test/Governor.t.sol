...
contract GovernorTest is Deployer {
    ...
    function testDelegateVote() public {
        ...
        uint256 govBalance = govToken.balanceOf(delegator);
        assertEq(govBalance, amount);

        assertEq(govToken.getVotes(delegator), 0);
        govToken.delegate(delegator);
        assertEq(govToken.getVotes(delegator), govBalance);

        address user2 = _getNextUserAddress();
        govToken.delegate(user2);
        assertEq(govToken.getVotes(delegator), 0);
        assertEq(govToken.getVotes(user2), govBalance);
    ...
    function testProposeErrorCase() public {
        ...
        uint256 amount = 10_000_000 ether - 2000 ether - 1 ether;
        stakeHub.delegate{ value: amount }(validator, false);
        uint256 shares = IStakeCredit(credit).balanceOf(delegator);
        assertEq(shares, amount);
        ...
        govToken.delegate(delegator);
        assertEq(govToken.getVotes(delegator), govBalance);
        console2.log("govBalance", govBalance);
        ...
        //
        // amount = 10000000 ether - 2000 ether;
        // govBalance = govToken.balanceOf(delegator);
        // console2.log("govBalance", govBalance);
        // assertEq(govBalance, amount);
        // assertEq(govToken.getVotes(delegator), govBalance);
        ...
        amount = 1 ether;
        stakeHub.undelegate(validator, amount);
        console2.log("voting power after undelegate", govToken.getVotes(delegator));
        assertEq(govToken.getVotes(delegator), govBalance - amount);
    }
    ...
    function testProposalNotApproved() public {
        ...
        assert(governor.proposeStarted());
        vm.deal(delegator, 20_000_000 ether);
        uint256 amount = 10_000_000 ether - 2000 ether;
        stakeHub.delegate{ value: amount }(validator, false);

        assertEq(govToken.getVotes(delegator), 0);
        govToken.delegate(delegator);
        assertEq(govToken.getVotes(delegator), amount);
        ...
        assertEq(govToken.getVotes(delegator2), amount);
