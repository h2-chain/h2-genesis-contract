...
contract SlashIndicatorTest is Deployer {
    ...
    function testMisdemeanor() public {
        ...
        validatorSet.deposit{ value: _deposit }(consensusAddrs[0]);
        assertEq(_incoming, validatorSet.getIncoming(consensusAddrs[0]));
        ...
        assertEq(0, validatorSet.getIncoming(consensusAddrs[0]));
        ...
        validatorSet.updateValidatorSetV2(newVals, newVotingPowers, newVoteAddrs);

        validatorSet.deposit{ value: 2 ether }(newVals[0]);
        assertEq(_incoming * 2, validatorSet.getIncoming(newVals[0]));
        ...
        assertEq(MISDEMEANOR_THRESHOLD, count);
        assertEq(0, validatorSet.getIncoming(newVals[0]));
        assertEq(_incoming, validatorSet.getIncoming(newVals[1]));
        assertEq(_incoming, validatorSet.getIncoming(newVals[2]));

        validatorSet.deposit{ value: _deposit }(newVals[1]);
        assertEq(_incoming * 2, validatorSet.getIncoming(newVals[1]));
        for (uint256 i; i < MISDEMEANOR_THRESHOLD; ++i) {
            vm.roll(block.number + 1);
            slashIndicator.slash(newVals[1]);
        }
        assertEq(_incoming, validatorSet.getIncoming(newVals[0]));
        assertEq(0, validatorSet.getIncoming(newVals[1]));
        assertEq(_incoming * 2, validatorSet.getIncoming(newVals[2]));
        ...
        assertEq(_incoming * 2, validatorSet.getIncoming(newVals[0]));
        assertEq(_incoming, validatorSet.getIncoming(newVals[1]));
        assertEq(0, validatorSet.getIncoming(newVals[2]));
        vm.stopPrank();
    }
    ...
    function testFelony() public {
        ...
        assertEq(0, count);
        assertEq(0, validatorSet.getIncoming(consensusAddrs[0]));
        assertEq(_incoming, validatorSet.getIncoming(consensusAddrs[1]));
        assertEq(_incoming, validatorSet.getIncoming(consensusAddrs[2]));
