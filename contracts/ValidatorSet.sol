...
contract ValidatorSet is IValidatorSet, System, IParamSubscriber {
    ...
    bytes public constant INIT_VALIDATORSET_BYTES = hex"";

    uint256 public constant INIT_NUM_OF_PROTONS = 25;
    ...
    // HIP-131 neutrons validator
    uint256 public numOfProtons; // 25
    uint256 public maxNumOfNeutrons; // 75
    uint256 public maxNumOfWorkingNeutrons; // 5

    // HIP-126 Fast Finality
    uint256 public constant INIT_SYSTEM_REWARD_RATIO = 625; // 625/10000 is 1/16
    // TODO Max SYSTEM_REWARD_BALANCE should be configurable
    ...
    // HIP-341 Validators can produce consecutive blocks
    ...
    function updateValidatorSetV2(
        ...
        for (uint256 i; i < currentValidatorSet.length; ++i) {
            ...
            if (incoming != 0) {
                ...
                // Add try-catch to prevent chain halt if StakeHub call fails
                try
                    ...
                    )
                {
                    ...
                    // If StakeHub call fails (e.g., validator not registered in StakeHub),
                    // send reward to SystemReward to prevent fund loss
                    address(uint160(SYSTEM_REWARD_ADDR)).transfer(incoming);
                }
            }
        }
     ...
     * including most of the protons and a few of the neutrons
     */
        ...
        returns (address[] memory, bytes[] memory)
    {
        ...
        uint256 _numOfProtons = numOfProtons > 0
            ? numOfProtons
            : INIT_NUM_OF_PROTONS;
        uint256 _shuffleInterval = 1000;

        address[] memory validators = getValidators();
        bytes[] memory voteAddrs = getVoteAddresses(validators);
        if (validators.length <= _numOfProtons) {
            return (validators, voteAddrs);
        }

        if ((validators.length - _numOfProtons) < _maxNumOfWorkingNeutrons) {
            _maxNumOfWorkingNeutrons = validators.length - _numOfProtons;
        }
        if (_maxNumOfWorkingNeutrons > 0) {
            uint256 shuffleNumber = block.number / _shuffleInterval;
            shuffle(
                validators,
                voteAddrs,
                shuffleNumber,
                _numOfProtons - _maxNumOfWorkingNeutrons,
                0,
                _maxNumOfWorkingNeutrons,
                _numOfProtons
            );
            shuffle(
                validators,
                ...
                _numOfProtons - _maxNumOfWorkingNeutrons,
                _maxNumOfWorkingNeutrons,
                validators.length - _numOfProtons + _maxNumOfWorkingNeutrons
            );
        }
        address[] memory miningValidators = new address[](_numOfProtons);
        bytes[] memory miningVoteAddrs = new bytes[](_numOfProtons);
        ...
        returns (uint256 workingValidatorCount)
    {
        ...
        uint256 _numOfProtons = numOfProtons > 0
            ? numOfProtons
            : INIT_NUM_OF_PROTONS;
        if (workingValidatorCount > _numOfProtons) {
            workingValidatorCount = _numOfProtons;
        }
    ...
    /**
     * @dev Enter maintenance for current validators.
     */
    ...
    /**
     * @dev Exit maintenance for current validators.
     */
    ...
    function updateParam(
        ...
        if (Memory.compareStrings(key, "burnRatio")) {
            ...
            uint256 _numOfProtons = numOfProtons;
            if (_numOfProtons == 0) {
                _numOfProtons = INIT_NUM_OF_PROTONS;
            }
            require(
                newMaxNumOfMaintaining < _numOfProtons,
                "the maxNumOfMaintaining must be less than numOfProtons"
            );
        ...
        } else if (Memory.compareStrings(key, "maxNumOfWorkingNeutrons")) {
            require(
                value.length == 32,
                "length of maxNumOfWorkingNeutrons mismatch"
            );
            uint256 newMaxNumOfWorkingNeutrons = BytesToTypes.bytesToUint256(
                32,
                value
            );
            require(
                newMaxNumOfWorkingNeutrons <= maxNumOfNeutrons,
                "the maxNumOfWorkingNeutrons must be not greater than maxNumOfNeutrons"
            );
            maxNumOfWorkingNeutrons = newMaxNumOfWorkingNeutrons;
        } else if (Memory.compareStrings(key, "maxNumOfNeutrons")) {
            require(value.length == 32, "length of maxNumOfNeutrons mismatch");
            uint256 newMaxNumOfNeutrons = BytesToTypes.bytesToUint256(
                32,
                value
            );
            maxNumOfNeutrons = newMaxNumOfNeutrons;
            if (maxNumOfWorkingNeutrons > maxNumOfNeutrons) {
                maxNumOfWorkingNeutrons = maxNumOfNeutrons;
            }
        } else if (Memory.compareStrings(key, "numOfProtons")) {
            require(value.length == 32, "length of numOfProtons mismatch");
            uint256 newNumOfProtons = BytesToTypes.bytesToUint256(32, value);
            require(
                newNumOfProtons > 0,
                "the numOfProtons must be greater than 0"
            );

            uint256 maxElectedValidators = IStakeHub(STAKE_HUB_ADDR)
                .maxElectedValidators();
            require(
                newNumOfProtons <= maxElectedValidators,
                "the numOfProtons must be less than maxElectedValidators"
            );

            numOfProtons = newNumOfProtons;
    ...
    /**
     * @dev With each shuffle interval blocks, there will be a partial rotation between protons and neutrons. Rotation is determined by this function
     */
    function shuffle(
