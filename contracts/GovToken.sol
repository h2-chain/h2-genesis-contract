...
{
    /*----------------- constants -----------------*/
    string private constant NAME = "H2 Governance Token";
    string private constant SYMBOL = "govH2";
    ...
    /*----------------- external functions -----------------*/
    /**
     * @dev Sync the account's govH2 amount to the actual H2 value of the StakingCredit he holds
    ...
    /**
     * @dev Batch sync the account's govH2 amount to the actual H2 value of the StakingCredit he holds
    ...
    /**
     * @dev delegate govH2 votes to delegatee
    ...
    function _sync(address stakeCredit, address account) internal {
        uint256 latestAmount = IStakeCredit(stakeCredit).getPooledToken(account);
        uint256 _mintedAmount = mintedMap[stakeCredit][account];

        if (_mintedAmount < latestAmount) {
            uint256 _needMint = latestAmount - _mintedAmount;
            mintedMap[stakeCredit][account] = latestAmount;
            _mint(account, _needMint);
        } else if (_mintedAmount > latestAmount) {
            uint256 _needBurn = _mintedAmount - latestAmount;
            mintedMap[stakeCredit][account] = latestAmount;
