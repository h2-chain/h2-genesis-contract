    ...
    GovernorPreventLateQuorumUpgradeable
{
     ...
     */
    uint256 private constant BLOCK_INTERVAL = 1 seconds; // TODO(Nathan): Only can be used to do initialize!
    uint256 private constant INIT_VOTING_DELAY = 0 hours * 100 / 75;
    uint256 private constant INIT_VOTING_PERIOD = 7 days * 100 / 75;
    uint256 private constant INIT_PROPOSAL_THRESHOLD = 200 ether; //  = 200 H2
    uint256 private constant INIT_QUORUM_NUMERATOR = 10; // for >= 10%

    // starting propose requires totalSupply of GovH2 >= 10000000 * 1e18
    uint256 private constant PROPOSE_START_GOV_SUPPLY_THRESHOLD = 10_000_000 ether;
    // ensures there is a minimum voting period (1 days) after quorum is reached
    uint64 private constant INIT_MIN_PERIOD_AFTER_QUORUM = uint64(1 days * 100 / 75);
    ...
    function _checkAndStartPropose() internal {
        if (!proposeStarted) {
            if (IGovToken(GOV_TOKEN_ADDR).totalSupply() < PROPOSE_START_GOV_SUPPLY_THRESHOLD) {
