...
contract StakeHub is SystemV2, Initializable, Protectable {
    ...
    struct StakeMigrationPackage {
        ...
        address refundAddress; // the address to refund the fund if migration failed
        uint256 amount; // the amount to be migrated(decimal: 18)
    }
    ...
    event Delegated(address indexed operatorAddress, address indexed delegator, uint256 shares, uint256 amount);
    event Undelegated(address indexed operatorAddress, address indexed delegator, uint256 shares, uint256 amount);
    event Redelegated(
    ...
    event Claimed(address indexed operatorAddress, address indexed delegator, uint256 amount);
    ...
    event MigrateSuccess(address indexed operatorAddress, address indexed delegator, uint256 shares, uint256 amount); // @dev deprecated
    event MigrateFailed(
        address indexed operatorAddress, address indexed delegator, uint256 amount, StakeMigrationRespCode respCode
    ); // @dev deprecated
    ...
    receive() external payable {
        // to prevent H2 from being lost
        if (_receiveFundStatus != _ENABLE) revert();
    }

    /**
     * @dev this function is invoked by H2 Parlia consensus engine during the hard fork
     */
    function initialize() external initializer onlyCoinbase onlyZeroGasPrice {
        transferGasLimit = 5000;
        minSelfDelegation = 2_025 ether;
        minDelegationChange = 1 ether;
        maxElectedValidators = 25;
        unbondPeriod = 7 days;
        redelegateFeeRate = 2;
        ...
        __Protectable_init_unchained(0x1e5afE0Fd369f92CA1b8039f773a2B64697Ca5E8);
    }
    ...
    function createValidator(
        ...
        uint256 delegation = msg.value - LOCK_AMOUNT; 
        if (delegation < minSelfDelegation) revert SelfDelegationNotEnough();
    ...
    function unjail(
        ...
        if (IStakeCredit(valInfo.creditContract).getPooledToken(operatorAddress) < minSelfDelegation) {
    ...
    function delegate(
        ...
        if (amount < minDelegationChange) revert DelegationAmountTooSmall();
        ...
        uint256 shares = IStakeCredit(valInfo.creditContract).delegate{ value: amount }(delegator);
        emit Delegated(operatorAddress, delegator, shares, amount);
    ...
    /**
     * @dev Undelegate token from a validator, fund is only claimable few days later
    ...
    function undelegate(
        ...
        uint256 amount = IStakeCredit(valInfo.creditContract).undelegate(delegator, shares);
        emit Undelegated(operatorAddress, delegator, shares, amount);
        ...
        enableReceivingFund
    {
        ...
        uint256 amount = IStakeCredit(srcValInfo.creditContract).unbond(delegator, shares);
        if (amount < minDelegationChange) revert DelegationAmountTooSmall();
        // check if the srcValidator has enough self delegation
        if (
            delegator == srcValidator
                && IStakeCredit(srcValInfo.creditContract).getPooledToken(srcValidator) < minSelfDelegation
        ) {
            revert SelfDelegationNotEnough();
        }

        uint256 feeCharge = amount * redelegateFeeRate / REDELEGATE_FEE_RATE_BASE;
        (bool success,) = dstValInfo.creditContract.call{ value: feeCharge }("");
        if (!success) revert TransferFailed();

        amount -= feeCharge;
        uint256 newShares = IStakeCredit(dstValInfo.creditContract).delegate{ value: amount }(delegator);
        emit Redelegated(srcValidator, dstValidator, delegator, shares, newShares, amount);
    ...
    /**
     * @dev Claim the undelegated Token from the pool after unbondPeriod
    ...
    /**
     * @dev Claim the undelegated Token from the pools after unbondPeriod
    ...
    function updateParam(string calldata key, bytes calldata value) external onlyGov {
        ...
        } else if (key.compareStrings("minSelfDelegation")) {
            if (value.length != 32) revert InvalidValue(key, value);
            uint256 newminSelfDelegation = value.bytesToUint256(32);
            if (newminSelfDelegation < 1000 ether || newminSelfDelegation > 100_000 ether) {
                revert InvalidValue(key, value);
            }
            minSelfDelegation = newminSelfDelegation;
        } else if (key.compareStrings("minDelegationChange")) {
            if (value.length != 32) revert InvalidValue(key, value);
            uint256 newminDelegationChange = value.bytesToUint256(32);
            if (newminDelegationChange < 0.1 ether || newminDelegationChange > 10 ether) {
                revert InvalidValue(key, value);
            }
     ...
     * @param index the index of the day to query(timestamp / 1 days)
     *
     * @return the validator's total pooled Token of the day
     */
    function getValidatorTotalPooledTokenRecord(address operatorAddress, uint256 index) external view returns (uint256) {
        if (!_validatorSet.contains(operatorAddress)) revert ValidatorNotExisted();
        return IStakeCredit(_validators[operatorAddress].creditContract).totalPooledTokenRecord(index);
    }
        ...
        )
    {
        ...
        for (uint256 i; i < count; ++i) {
            ...
            votingPowers[i] = valInfo.jailed ? 0 : IStakeCredit(valInfo.creditContract).totalPooledToken();
    ...
    function addNodeIDs(
    ...
    ) external whenNotPaused notInBlackList validatorExist(_hip563MsgSender()) {
        ...
        address operatorAddress = _hip563MsgSender();
    ...
    function removeNodeIDs(
        ...
        address validator = _hip563MsgSender();
    ...
    function _claim(address operatorAddress, uint256 requestNumber) internal validatorExist(operatorAddress) {
        uint256 amount = IStakeCredit(_validators[operatorAddress].creditContract).claim(msg.sender, requestNumber);
        emit Claimed(operatorAddress, msg.sender, amount);
    }

    function _hip410MsgSender() internal view returns (address) {
